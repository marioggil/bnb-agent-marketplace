...
- [ ] **T5 — GREEN: implement `flagged_data_stale(session, threshold_hours=_STALE_THRESHOLD_HOURS) -> bool`.** Extend `app/services/compliance_refresh.py`. Single SQL: `SELECT max(updated_at) FROM flagged_addresses`. If `None` (empty mirror) → `True`. Else → `(datetime.now(UTC) - max_ts).total_seconds() > threshold_hours * 3600`. Run `uv run pytest tests/test_compliance_refresh.py -k flagged_data_stale`. Expected: passes. <!-- sdd-owner: implementation -->

- [ ] **T6 — RED: `test_run_compliance_refresh_chains_both_phases` + short-circuit + combined-report.** In `tests/test_compliance_refresh.py` (or a new `test_orchestrator.py` if imports get long):
