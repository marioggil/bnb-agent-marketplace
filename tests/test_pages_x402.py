...
def _seed_one(session, token_id: int = 1, wallet: str | None = "0x" + "77" * 20) -> str:
    ...
    session.add(AgentCache(
        ...
        registry_address=BSC_IDENTITY_REGISTRY, name="Alpha", agent_wallet=wallet,
        agent_url=_AGENT_URL, supported_protocols=[], cross_chain_versions=[], raw={},
    ))
    session.commit()
    return aid
